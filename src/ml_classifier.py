    ...
    def train_models(self, test_size=0.2):
        ...
        for condition in conditions:
            ...
            try:
                ...
                # === Create folder for visual outputs if not exists ===
                os.makedirs("visuals", exist_ok=True)

                # === Visualization: Confusion Matrix ===
                plt.figure(figsize=(5, 4))
                sns.heatmap(cm, annot=True, cmap="Blues", fmt="d")
                plt.title(f"{condition.upper()} - Confusion Matrix")
                plt.xlabel("Predicted")
                plt.ylabel("Actual")
                plt.tight_layout()
                plt.savefig(f"visuals/{condition}_confusion_matrix.png")
                plt.close()
                ...
                plt.title(f"{condition.upper()} - ROC Curve")
                plt.xlabel("False Positive Rate")
                plt.ylabel("True Positive Rate")
                plt.legend()
                plt.tight_layout()
                plt.savefig(f"visuals/{condition}_roc_curve.png")
                plt.close()
                ...
                plt.title(f"{condition.upper()} - Feature Importance")
                plt.xlabel("Importance")
                plt.ylabel("Feature")
                plt.tight_layout()
                plt.savefig(f"visuals/{condition}_feature_importance.png")
                plt.close()
